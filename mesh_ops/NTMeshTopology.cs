...
namespace gs
{
    ...
    public class NTMeshTopology
    {
        ...
        public NTMeshTopology(NTMesh3 mesh)
        {
            Mesh = mesh;
        }
        ...
        public NTEdgeLoop[] Loops;
        public NTEdgeSpan[] Spans;
        ...
        void find_crease_edges(double angle_tol)
        {
            ...
            foreach ( int eid in Mesh.EdgeIndices() ) {
                var edgeTris = Mesh.EdgeTrianglesItr(eid).ToList();
                if (edgeTris.Count <= 1) {
                    BoundaryEdges.Add(eid);
                    continue;
                }

                for (int i = 0; i < edgeTris.Count - 1; i++)
                {
                    var ni = Mesh.GetTriNormal(edgeTris[i]);
                    for (int j = i + 1; j < edgeTris.Count; j++)
                    {
                        var nj = Mesh.GetTriNormal(edgeTris[j]);

                        if (Math.Abs(ni.Dot(nj)) < dot_tol)
                        {
                            CreaseEdges.Add(eid);
                        }
                    }
        ...
        public NTMesh3 MakeElementsMesh(Polygon2d spanProfile, Polygon2d loopProfile)
        {
            var result = new NTMesh3();
            validate_topology();

            foreach (var span in Spans) 
            {
                var curve = span.ToCurve(Mesh);
                TubeGenerator tubegen = new TubeGenerator(curve, spanProfile);
                NTMeshEditor.Append(result, tubegen.Generate().MakeNTMesh());
            }
