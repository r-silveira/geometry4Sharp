...
namespace g4
{
	...
	public partial class NTMesh3 : IDeformableMesh
	{
        ...
        public NTMesh3(bool bWantNormals = true, bool bWantColors = false, bool bWantTriGroups = false, bool bWantVtxGroups = false)
		{
			allocate(bWantNormals, bWantColors, bWantTriGroups, bWantVtxGroups);
		}
		
		
		public NTMesh3(MeshComponents flags) : 
			this((flags & MeshComponents.VertexNormals) != 0,  (flags & MeshComponents.VertexColors) != 0,
				 (flags & MeshComponents.FaceGroups) != 0, (flags & MeshComponents.VertexGroups) != 0)
		{
		}


		private void allocate(bool bWantNormals, bool bWantColors, bool bWantTriGroups, bool bWantVtxGroups)
		{
		...
		public void Copy(NTMesh3 copy, bool bNormals = true, bool bColors = true)
		{
            ...
            if (copy.HasVertexGroups)
            {
                vertex_groups = new DVector<int>(copy.vertex_groups);
            }
		...
		public int MaxVertexID { get { return vertices_refcount.max_index; } }
		public int MaxTriangleID { get { return triangles_refcount.max_index; } }
		public int MaxEdgeID { get { return edges_refcount.max_index; } }
		public int MaxGroupID { get { return max_group_id; } }
        ...
        public bool HasVertexGroups { get { return vertex_groups != null; } }
		...
		{ 
			return HasTriangleGroups ? (triangles_refcount.isValid(tid) ? triangle_groups[tid] : 0) : -1;
		}


        public int GetVertexGroup(int vid)
        {
            return HasVertexGroups ? (vertices_refcount.isValid(vid) ? vertex_groups[vid] : 0) : -1;
        }
        ...
        public void SetVertexGroup(int vid, int group_id)
        {
            if (HasVertexGroups)
            {
                debug_check_is_vertex(vid);
                vertex_groups[vid] = group_id;
                max_vertex_group_id = Math.Max(max_vertex_group_id, group_id + 1);
                updateTimeStamp(false);
            }
        }
		...
		/// <summary>
		/// Pre-allocates memory for a specified number of vertices to improve performance.
		/// </summary>
		/// <remarks>
		/// Call this method before adding a large number of vertices to avoid multiple
		/// internal array re-allocations, which can be a performance bottleneck.
		/// </remarks>
		/// <param name="vertexCount">The number of vertices to reserve capacity for.</param>
		public void ReserveVertices(int vertexCount)
		{
            ...
            if (HasVertexGroups)
            {
                vertex_groups.resize(vertexCount);
            }
        ...
        /// <summary>
        /// Pre-allocates memory for a specified number of triangles to improve performance.
        /// </summary>
        /// <remarks>
        /// Call this method before adding a large number of triangles to avoid multiple
        /// internal array re-allocations.
        /// </remarks>
        /// <param name="triangleCount">The number of triangles to reserve capacity for.</param>
        public void ReserveTriangles(int triangleCount)
		{
			...
			int estimatedEdges = triangleCount + (triangleCount / 2);
			edges.resize(estimatedEdges * 2);
		}


		public int AppendVertex(Vector3d v, int gid = -1) {
			return AppendVertex(new NewVertexInfo() {
				v = v, bHaveC = false, bHaveUV = false, bHaveN = false, gid = gid
			});
		}
		...
		public int AppendVertex(NewVertexInfo info)
		{
            ...
            if (HasVertexGroups)
            {
                vertex_groups.insert(info.gid, vid);
                max_vertex_group_id = Math.Max(max_vertex_group_id, info.gid + 1);
            }
        ...
        public void EnableVertexGroups(int initial_group = 0)
        {
            if (HasVertexGroups)
			{
                return;
            ...
            vertex_groups = new DVector<int>();
            vertex_groups.resize(MaxVertexID);
            for (int i = 0; i < MaxVertexID; ++i)
			{
                vertex_groups[i] = initial_group;
            }
        ...
        public void DiscardVertexGroups()
        {
            vertex_groups = null;
            max_vertex_group_id = 0;
        }
